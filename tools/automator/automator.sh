...
get_opts() {
  if opt="$(getopt -o i -l branch:,org:,repo:,title:,body:,user:,email:,script-path:,cmd:,token-path:,token: -n "$(basename "$0")" -- "$@")"; then
    eval set -- "$opt"
  else
...
main() {
  ...
  if ! git diff --quiet --exit-code; then
    ...
    git show --shortstat
    git push --force "https://$user:$token@github.com/$user/$repo.git" "HEAD:$branch-automator"
